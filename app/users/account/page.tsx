...
import { FriendshipPayload, TabValue, UploadResult } from "@/types/types";
...
const Account = () => {
  ...
  const { setFriends, setPendingRequests, setFriendshipStatus, friends, pendingRequests } = useAccountStore(state => ({
  ...
  const handleTabChange = (tabValue: TabValue) => {
    ...
    const tabIndexMap: Record<TabValue, number> = {
      "about": 0,
      "friends": 1,
      "posts": 2
    };
  ...
  }, [selectedTabIndex, friends.length, pendingRequests.length, user?.id, setFriends, setPendingRequests]);
  ...
  const handleDeclineRequest = async (friendRequestId: string) => {
    try {
  ...
  const handleRemoveFriend = async (friendRequestId: string) => {
    try {
          ...
          {loading ? (
            ...
            <Card>
              ...
              <CardContent>
                {pendingRequests && pendingRequests.length > 0 && (
                  <>
                    <div className="space-y-2">
                      {pendingRequests.map((request: FriendshipPayload) => (
                ...
                {friends.length > 0 ? (
                  <div className="space-y-2">
                    {friends.map((friendship: FriendshipPayload) => (
                      // ... Render each friend
